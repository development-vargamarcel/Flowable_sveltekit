<script lang="ts">
	...
	function toggleColumnSelection(gridName: string, columnName: string) {
		const existing = selectedColumns.find((c: { gridName: string; columnNames: string[] }) => c.gridName === gridName);
		if (existing) {
	...
	function isColumnSelected(gridName: string, columnName: string): boolean {
		const gridTarget = selectedColumns.find((c: { gridName: string }) => c.gridName === gridName);
